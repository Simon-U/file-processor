...
path_test_data = Path(__file__).parent.parent / 'Test_data' / 'suh1212.dat'
...
path_dict = {
    ...
    'data_header': Path(__file__).parent.parent / 'Schema' / 'CSV' / 'v_3.7.0' / 'DE' / ('definition_record_' + 'SU' +
                                                                                         '.csv'),
    'data_records': Path(__file__).parent.parent / 'Schema' / 'CSV' / 'v_3.7.0' / 'DE' / ('definition_data_' + 'SU' +
                                                                                          '.csv')
}
...
def test_write_geo_equals():
    ...
    assert sum(raw_data[(sel_data == '1')] == write_geo(path_dict.get('geo_structure'), df_geo, dict_geo, "CSV",
                                                        "v_3.7.0", "DE")) ==\
           len(raw_data[(sel_data == '1')]), "Written geo data and raw data are not equal"


def test_write_data_header_number_rows():
    dict_data_header = {
        'Marktcode': '00000',
        "Produktcode": "123456789",
        "Summenname": "NEWDEVE",
    ...
    assert len(write_data_header(path_dict.get('data_header'), df_data, dict_data_header, "CSV", "SU", "v_3.7.0", "DE")
...
def test_write_data_header_equal():
    ...
    assert write_data_header(path_dict.get('data_header'), df_data, dict_data_header, "CSV", "SU", "v_3.7.0", "DE")[0] \
...
def test_write_data_length_string():
    ...
    # Test: The sum of identical rows from the written data to the written must be the same as the length
    # of the raw data. If not, it means some rows are not correctly printed
    assert len(write_data(path_dict.get('data_records'), df_data, dict_data, "CSV", "SU", "v_3.7.0", "DE")[0]) == 140,\
...
def test_write_data_equals():
    ...
    # Test: The sum of identical rows from the written data to the written must be the same as the length
    # of the raw data. If not, it means some rows are not correctly printed
    assert sum(raw_data[(sel_data == '7')] == write_data(path_dict.get('data_records'), df_data, dict_data, "CSV", "SU"
                                                         , "v_3.7.0", "DE")) == \
