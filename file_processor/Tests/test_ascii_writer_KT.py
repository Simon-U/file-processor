...
path_test_data = Path(__file__).parent.parent / 'Test_data' / 'kth1212.dat'
...
path_dict = {
    ...
    'data_header': Path(__file__).parent.parent / 'Schema' / 'CSV' / 'v_3.7.0' / 'DE' / ('definition_record_' + 'KT' +
                                                                                         '.csv'),
    'data_records': Path(__file__).parent.parent / 'Schema' / 'CSV' / 'v_3.7.0' / 'DE' / ('definition_data_' + 'KT' +
                                                                                          '.csv')
}
...
def test_write_geo_length_string():
    ...
    assert len(write_geo(path_dict.get('geo_structure'), df_geo, dict_geo, "CSV", "v_3.7.0", "DE")[0]) == 140, \
        "Written geo data length is not 140"
...
def test_write_geo_equals():
    ...
    assert sum(raw_data[(sel_data == '1')] == write_geo(path_dict.get('geo_structure'), df_geo, dict_geo, "CSV",
                                                        "v_3.7.0", "DE")) == len(raw_data[(sel_data == '1')]), \
        "Written geo data and raw data are not equal"


def test_write_data_header_number_rows():
    # The first value is column name and the second string.
    dict_data_header = {
    ...
    assert len(write_data_header(path_dict.get('data_header'), df_data, dict_data_header, "CSV", "KT", "v_3.7.0", "DE")
...
def test_write_data_header_equal():
    ...
    df_data = read_file(path_test_data, to_be_merged=True)
    raw_data = np.genfromtxt(path_test_data, dtype=str, delimiter=140)
    assert write_data_header(path_dict.get('data_header'), df_data, dict_data_header, "CSV", "KT", "v_3.7.0", "DE")[0]\
...
def test_write_data_length_string():
    ...
    # Test: The sum of identical rows from the written data to the written must be the same as the length
    # of the raw data. If not, it means some rows are not correctly printed
    assert len(write_data(path_dict.get('data_records'), df_data, dict_data, "CSV", "KT", "v_3.7.0", "DE")[0]) == 140,\
...
def test_write_data_equals():
    ...
    # Test: The sum of identical rows from the written data to the written must be the same as the length
    # of the raw data. If not, it means some rows are not correctly printed
    assert sum(raw_data[(sel_data == '4')] == write_data(path_dict.get('data_records'), df_data, dict_data, "CSV", "KT", "v_3.7.0", "DE")) == \
           len(raw_data[(sel_data == '4')]), "Written data is not equal to the raw data"
